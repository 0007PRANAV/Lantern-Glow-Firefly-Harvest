...
// Leaver Penalty Interceptor Mechanics (Shared Core Routing Logic)
function triggerQuitConfirmation(contextMessagePrefix) {
    changeView('confirm-quit');
    
    let warning = document.getElementById('quit-warning-text');
    let projectedQuits = activeUser.quitCount + 1;
    if (projectedQuits >= 3) {
        warning.innerHTML = `<span class="red-text"><i class="fas fa-exclamation-triangle"></i> PENALTY WARNING:</span> ${contextMessagePrefix} Leaving now will issue a <strong>3-MINUTE LOCKOUT PENALTY</strong> instantly!`;
    } else {
...
// In-Game Global Header Abandon Button Click Hook
globalQuitBtn.addEventListener('click', () => {
    state = 'PAUSE';
    overlay.classList.add('active');
    triggerQuitConfirmation("Quitting this match early registers a system infraction.");
});

// New Settings Panel "LEAVE GAME" Button Hook Action
document.getElementById('settings-leave-game-btn').addEventListener('click', () => {
    if (state !== 'PLAY' && state !== 'PAUSE') {
        alert("No active game loop matrix timeline is currently executing!");
        return;
    }
    triggerQuitConfirmation("Leaving the run early directly via system settings registers an infraction.");
});
...
document.getElementById('btn-confirm-quit-yes').addEventListener('click', () => {
    ...
    if (activeUser.quitCount >= 3) {
        activeUser.penaltyUntil = Date.now() + 180000; // 3-minute lockout delay assignment
...
// Settings Modal Routing Navigation Configurations
globalSettingsBtn.addEventListener('click', () => {
    if (!activeUser) { alert("Gain core system entry first!"); return; }
    if (state === 'PLAY') state = 'PAUSE';
    overlay.classList.add('active');
    renderExitHistory();
    changeView('settings');
...
// System Maintenance Purge Controls
...
// Canvas Interactive Tap Event Capture Pipelines
...
// Engine Entity Blueprint Structures
class Entity {
...
document.getElementById('btn-play').addEventListener('click', () => {
    ...
    state = 'PLAY'; score = 0; multiplier = 1; maxCombo = 1; wave = 1; darkness = 0; overdrive = 0;
...
// Session Auto-Login Verification Lift-off Hook
